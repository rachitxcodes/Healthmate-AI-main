...
export default function Sidebar() {
    ...
    return (
        <div className={`fixed left-0 top-0 z-40 h-screen py-4 pl-3 pr-2 transition-all duration-300 ease-in-out ${isExpanded ? "w-64" : "w-[5.5rem] lg:w-24"}`}>
            <div className={`h-full bg-white/80 backdrop-blur-xl rounded-3xl flex flex-col py-7 relative shadow-[0_4px_30px_rgba(244,63,94,0.06)] border border-rose-100/60 ${isExpanded ? "px-4" : "items-center"}`}>

                {/* Logo / Brand */}
                <div className={`flex items-center gap-3 mb-8 w-full relative group ${isExpanded ? "justify-start px-2" : "justify-center flex-col"}`}>
                    <div className="bg-gradient-to-br from-primary to-rose-600 p-1.5 rounded-2xl flex items-center justify-center flex-shrink-0 shadow-[0_4px_12px_rgba(244,63,94,0.3)]">
                    ...
                    {isExpanded ? (
                        ...
                        <span className="text-primary font-bold text-[9px] tracking-widest uppercase mt-1.5">HM</span>
                    )}

                    {/* Expand Toggle */}
                    <button onClick={() => setIsExpanded(!isExpanded)} className={`absolute ${isExpanded ? "-right-8" : "-right-5"} top-2 bg-white hover:bg-rose-50 rounded-full p-1.5 shadow-md border border-rose-100 transition-all z-10`}>
                        <ChevronRight size={14} className={`text-primary transition-transform duration-300 ${isExpanded ? "rotate-180" : ""}`} strokeWidth={3} />
                    </button>
                </div>
                    ...
                    {navLinks.map((link) => {
                        ...
                        return (
                            <Link
                                ...
                                ${isExpanded ? "w-full rounded-2xl px-3.5 justify-start gap-3.5 hover:bg-rose-50/80 hover:text-primary" : "w-11 justify-center rounded-xl hover:bg-rose-50/80 hover:text-primary"}
                                ${isActive && isExpanded ? "bg-gradient-to-r from-primary to-rose-500 text-white shadow-[0_4px_14px_rgba(244,63,94,0.35)] hover:shadow-[0_6px_18px_rgba(244,63,94,0.4)] hover:bg-none" : ""}
                                ${isActive && !isExpanded ? "bg-gradient-to-br from-primary to-rose-500 text-white shadow-[0_4px_14px_rgba(244,63,94,0.35)]" : ""}
                                ${!isActive ? "text-slate-400 hover:text-primary" : ""}`}
                            >
                                ...
                                {isActive && !isExpanded && (
                                    <motion.div layoutId="sidebar-active" className="absolute left-[-17px] lg:left-[-17px] w-[3px] h-5 bg-primary rounded-r-full" />
                                )}
                            </Link>
                        );
                    })}
                ...
                <div className={`flex flex-col gap-2 mt-auto w-full ${isExpanded ? "" : "items-center px-2"}`}>
                    <Link to="/settings" title={!isExpanded ? "Settings" : ""} className={`flex items-center h-11 transition-all duration-200 ${isExpanded ? "w-full rounded-2xl px-3.5 justify-start gap-3.5 hover:bg-rose-50/80 text-slate-400 hover:text-primary" : "w-11 justify-center rounded-xl hover:bg-rose-50/80 text-slate-400 hover:text-primary"}`}>
